...
export default function Editor() {
  const [updatedX, setUpdatedX] = useState<number>(0)
  const [updatedY, setUpdatedY] = useState<number>(0)
  const [allX, setAllX] = useState<Array<number>>([])
  const [allY, setAllY] = useState<Array<number>>([])
  ...
  const [croperRectState, setCroperRectState] = useRecoilState(croperState)
  ...
  function updateCoordinates(lines: LineGroup) {
    lines.forEach(line => {
      ...
      line.pts.forEach(pts => {
        const xPointValue = [...allX, pts.x]
        setAllX(xPointValue)
        const yPointValue = [...allY, pts.y]
        setAllY(yPointValue)
      })
      console.log('medianY:', updatedY)
      console.log('medianX:', updatedX)
    })
    // console.log('test median x:', getMedian(allX))
    // console.log('test median y:', getMedian(allY))
    // setUpdatedX(getMedian(allX))
    // setUpdatedY(getMedian(allY))
  }
  ...
  useEffect(() => {
    if (curLineGroup.length > 0) {
      console.log('........lol.......')
      updateCoordinates(curLineGroup)
      const medianX = Math.max(getMedian(allX) - 543 / 2, 0)
      const medianY = Math.max(getMedian(allY) - 619 / 2, 0)
      setUpdatedX(medianX)
      setUpdatedY(medianY)
      const newRect = { ...croperRectState, x: medianX, y: medianY }
      setCroperRectState(newRect)
    } else {
      setAllX([])
      setAllY([])
      setUpdatedX(0)
